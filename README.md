...
- **日内风控**：虽然周度轮动，但每日盘后检查 Hard-Exit（跌破支撑）与 Soft-Trim（涨幅过热），并统一在**下一交易日开盘**执行风控减仓/平仓。

### 3. 回测结果

> **重要说明（2026-04 timing audit）**：`src/backtester.py` 中的日度风控路径此前存在时序错误——使用 `t` 日收盘后才完整可知的 `close / RSI / LogBias / EMA` 信号，却按同一交易日 `open` 执行卖出。该问题现已修正为：**`t` 日收盘判定，`t+1` 日开盘执行**。这次修正消除了前视界，但也显著下修了历史业绩，说明旧结果高估了策略表现。
>
> 使用 `results/best_parameters.csv` 中的既有最优参数、在同一数据窗重新对比后的结果如下：
>
> - **训练集（2009–2019）**：年化 **4.83% → 0.92%**，Sharpe **0.54 → 0.14**，最大回撤 **-10.35% → -22.32%**

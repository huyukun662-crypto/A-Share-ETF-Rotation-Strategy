...
This folder stores the matplotlib charts (150 dpi) generated by
`python -m src.pipeline`.  `train_test_comparison.png` is committed
to the repo so that GitHub can render it directly in the top-level
README; the rest are reproducible on demand.

该目录存放由 `python -m src.pipeline` 生成的 matplotlib 图表
（150 dpi）。其中 `train_test_comparison.png` 已提交至仓库，便于
顶层 README 直接展示；其余图表可按需重新生成。

## 图表清单 · Charts

### `train_test_comparison.png` ✅ committed
...
![train_test_comparison.png](train_test_comparison.png)
